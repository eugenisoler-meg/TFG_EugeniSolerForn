...
import { Sortida, User } from "@/constants/model";
...
export default function SortidesScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const s = JSON.parse(
          await getSortidesByUnitatID(user.afiliat_id, unitat_id),
        ) as Sortida[];
  ...
  const renderItem = ({ item }: { item: Sortida }) => {
    ...
    return (
      <SortidaCard
        item={item}
        generateText={AI.generateSortidaText}
        onEdit={(sortida) =>
          router.push({
            pathname: "./sortida-form",
            params: {
              unitat_id: sortida.unitat_id,
              sortida: JSON.stringify(sortida),
            },
          })
        }
      />
  ...
  const renderEmpty = () => (
    <ThemedView style={styles.empty}>
      <ThemedText type="subtitle">
        Encara no has planejat cap sortida
      </ThemedText>
    </ThemedView>
  );
  ...
  if (error) return <ErrorScreen message={error ?? "Error desconegut."} />;
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <AddIcon
        onPress={() =>
          router.push({
            pathname: "/(app)/(aeig)/(unitat)/sortida-form",
            params: { unitat_id },
          })
        }
      />
