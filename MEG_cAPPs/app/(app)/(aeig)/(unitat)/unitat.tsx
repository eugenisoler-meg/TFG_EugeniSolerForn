...
import FuncionsUnitat, {
    FuncioUnitat,
} from "@/components/aeig/unitat/unitat-infants";
...
export default function UnitatIndex() {
  ...
  const { unitat_id, funcio } = useLocalSearchParams<{
    unitat_id?: string;
    funcio?: string;
  }>();
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (!unitat_id) return;
        const response = await DATABASE.getFuncionsByUnitatID(
          user.afiliat_id,
          unitat_id,
        );
  ...
  const page = {
    ...
    key: "cap_grups",
    selectable: true,
    data: [funcions.filter((f) => f.afiliat_id === user.afiliat_id)],
  };
  return (
    <>
