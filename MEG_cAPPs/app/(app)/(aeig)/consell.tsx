...
// Group funcions by branca (excluding equip agrupament)
      ...
      <View style={styles.tableContainer}>
        <View style={styles.table}>
          ...
          <View style={[styles.rowItem, styles.header]}>
            ...
            {showUnitat && (<Text style={[styles.cell, styles.headerText]}>Unitat</Text>)}
          </View>
          {/* Data rows */}
          {data.map((item) => {
          const fullName = [item.afiliat?.nom, item.afiliat?.cognoms].filter(Boolean).join(' ');
          ...
          return (
            <View key={item.funcio_id} style={styles.rowItem}>
              ...
              {item.unitat?.nom && (<Text style={styles.cell}>{item.unitat.nom}</Text>)}
            </View>
          );
        })}
      </View>
  ...
  const brancaOrder = ['castors_lludrigues', 'llops_daines', 'rangers_noies_guia', 'pioners_caraveles', 'trucs'];
  ...
  const equipAgrupamentFunctions = funcions.filter(f => f.grup?.endsWith('_equip_agrupament'));

  return (
    <ScrollView style={styles.container}>
      <ThemedText type="title" style={styles.title}>Consell d'Agrupament</ThemedText>
      ...
      {sortedBrancaEntries.map(([branca, brancaFuncions]) => {
        ...
        return (
          <Collapsible key={branca} title={MAP_LABELS[branca] || branca} headerColor={headerColor}>
            <View style={styles.brancaContent}>
              {capGrups.length > 0 && renderTable(capGrups)}
              {capGrups.length === 0 && (
                <ThemedText style={styles.emptyText}>No hi ha caps de grup per aquesta branca</ThemedText>
              )}
            </View>
          </Collapsible>
        );
