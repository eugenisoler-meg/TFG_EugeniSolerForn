...
import AgrupamentCard, {
  AgrupamentDetails,
} from "@/components/aeig/agrupament-details";
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        for (let i = 0; i < AGRUPAMENTS_ID.length; i++) {
          const aeig = JSON.parse(
            await DATABASE.getAgrupamentByID(
              user.afiliat_id,
              AGRUPAMENTS_ID[i],
              true,
            ),
          ) as AgrupamentDetails;
        ...
        renderItem={({ item }) => (
          <AgrupamentCard
            ...
            onPress={() =>
              router.push({
                pathname: "./aeig",
                params: { agrupament_id: item.agrupament_id },
              })
            }
          />
        )}
